 ...
 * @brief 信息流广告渲染模式
 */
typedef NS_ENUM(NSInteger, MedproFHHXNativeAdRenderMode) {
    /// 模板渲染（SDK 渲染 UI，默认）
    /// 使用 adView 获取 SDK 渲染好的广告视图
    MedproFHHXNativeAdRenderModeTemplate = 0,
    
    /// 自渲染（媒体渲染 UI）
    /// 使用 renderData 获取数据，自行创建视图后调用 renderData 的绑定方法
    MedproFHHXNativeAdRenderModeCustom = 1,
};

/**
 * @brief 自渲染信息流的视频渲染方式
 *
 * @discussion 仅 renderMode = MedproFHHXNativeAdRenderModeCustom 时有效
 */
typedef NS_ENUM(NSInteger, MedproFHHXNativeVideoRenderMode) {
    /// SDK 提供播放视图（默认），取 renderData.mediaView 使用
    MedproFHHXNativeVideoRenderModeSDK = 0,
    
    /// 开发者自行提供播放器，renderData.mediaView 为 nil，
    /// 视频素材见 renderData 的 videoUrl / videoCoverUrl / videoDuration / videoSize
    MedproFHHXNativeVideoRenderModeCustom = 1,
};
 ...
 *
 * @discussion
 * 仅 videoRenderMode = MedproFHHXNativeVideoRenderModeCustom 时使用。
 * 广告滑出屏幕、App 切后台、用户点击跳转等场景下 SDK 会回调暂停/恢复，
 * 请在回调里操作自己的播放器。回调均在主线程。
 */
@protocol MedproFHHXNativeAdCustomVideoDelegate <NSObject>

@required

/// 恢复播放（广告重新可见、App 回前台、跳转页关闭等）
- (void)nativeAdRequestResumeVideo:(MedproFHHXNativeAd *)nativeAd;

/// 暂停播放（广告不可见、App 进后台、用户跳转等）
- (void)nativeAdRequestPauseVideo:(MedproFHHXNativeAd *)nativeAd;

@optional

/// 设置静音，nativeAd.videoMuted 变化时回调。
/// enableDefaultAudioSessionSetting 为 YES 时，取消静音前 SDK 会先配置 AVAudioSession，媒体只需同步 player.muted。
- (void)nativeAd:(MedproFHHXNativeAd *)nativeAd requestMuteVideo:(BOOL)muted;

/// 停止播放并释放资源，广告 close 时回调，之后不会再有恢复回调
- (void)nativeAdRequestStopVideo:(MedproFHHXNativeAd *)nativeAd;

@end

@interface MedproFHHXNativeAd : NSObject <MedproFHHXBidNotifiable>
 ...
 */
@property (nonatomic, weak, nullable) id<MedproFHHXNativeAdDelegate> delegate;
 ...
 * @discussion
 * 必须在调用 loadAd 之前设置。
 * - MedproFHHXNativeAdRenderModeTemplate（默认）：SDK 渲染 UI，通过 adView 获取
 * - MedproFHHXNativeAdRenderModeCustom：媒体自渲染，通过 renderData 获取数据
 *
 * @default MedproFHHXNativeAdRenderModeTemplate
 */
@property (nonatomic, assign) MedproFHHXNativeAdRenderMode renderMode;

/**
 * @brief 视频渲染方式（仅自渲染模式有效）
 *
 * @discussion
 * loadAd 前设置。默认 MedproFHHXNativeVideoRenderModeSDK，取 renderData.mediaView 即可。
 * 设为 Custom 时自行播放视频，绑定时使用
 * bindWithContainer:clickableViews:customVideoView:videoDelegate:
 */
@property (nonatomic, assign) MedproFHHXNativeVideoRenderMode videoRenderMode;
 ...
 * @discussion
 * 加载成功后可获取此数据用于自渲染 UI。
 * 仅当 renderMode == MedproFHHXNativeAdRenderModeCustom 时有值。
 * 模板渲染模式下返回 nil。
 *
 * 数据包含：
 * - 文本信息（标题、描述、按钮文案）
 ...
 *
 */
@property (nonatomic, strong, readonly, nullable) MedproFHHXNativeAdRenderData *renderData;
